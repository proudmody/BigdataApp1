create or replace procedure P_zdry_lq is  --重点人员两抢（抢劫、抢夺）
...
begin
     ...
     for user_info in cur_user_info  loop
          ...
          --根据年龄积分
          if vAge = 20 or vAge = 21 then
             vScore_age := 10.0;
          elsif vAge = 19 or vAge = 22 or vAge =23 then
             vScore_age := 8.0;
          elsif vAge = 18 or vAge =24 then
             vScore_age := 6.0;
          elsif vAge = 17 or vAge = 25 or vAge = 26 then
             vScore_age := 5.0;
          elsif vAge between 27 and 35 then
             vScore_age := 3.0;
          elsif vAge = 16 then
             vScore_age := 3.0;
          elsif vAge between 36 and 200 then
             vScore_age := 1.0;
          elsif vAge = 15 then
             vScore_age := 1.0;
          elsif vAge < 15 then
          ...
          vSql := 'select count(1) from d_zdry_gwdy_lq t where t.dm = '''||user_info.jg||'''' ;
          execute immediate vSql into isCountNum;
          if isCountNum >0 then
             vSql := 'select t.score from d_zdry_gwdy_lq t where t.dm = '''||user_info.jg||'''' ;
          ...
          --计算两抢前科 数据
          vSql := 'select count(*) from wsba.hx_a_ajjbqk@ywhj132 w where (zabmc like ''%抢夺%'' or zabmc like ''%抢劫%'') and AJZTDM in (''204'', ''299'') and w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = '''||user_info.zjhm||''')';
          ...
          if vNum_this > 0 then
            for v_criminal_record in (select guid from wsba.hx_a_ajjbqk@ywhj132 w where (zabmc like '%抢夺%' or zabmc like '%抢劫%') and AJZTDM in ('204', '299') and w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = user_info.zjhm)) loop
                begin
                    ...
                    values (user_info.rid,user_info.zjhm,user_info.xm, 5,vLastTime, '两抢前科',
          ...
          insert into t_zdry_score_result (ZJHM,SCORE2,SCORE_OF_QK2,ZHGXSJ_LQ)
     ...
     insert into T_ZDRY_LOG (STARTTIME, ENDTIME, TOTALTIME, TABLENAME,RECORDCOUNT, DETAILACTION)
            values(vStartTime, vEndTime, round(to_number(to_date(vEndTime,'yyyymmddhh24miss') - to_date(vStartTime,'yyyymmddhh24miss'))*24*60*60), 'PRIAPWEB.t_zdry_jbxx_old'，vCount, '两抢数据');
     commit;
