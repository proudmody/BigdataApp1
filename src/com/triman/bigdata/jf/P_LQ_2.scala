  ...
  def compute(sqlContext:SQLContext,T_ZDRY_JBXX:DataFrame,HX_A_AJJBQK:DataFrame,VW_WSBA_HX_R_XYRC:DataFrame
    ...
    val pqlbQk = HX_A_AJJBQK.rdd.filter{
      r=>
        ...
        if ((zabmc.contains("抢夺")||zabmc.contains("抢劫"))&&((ajztdm=="204")||(ajztdm=="299"))) true
        else false
    }.map{
      r=>
...
//    tJoinAreaJoinQkJoinWbJoinSb.printSchema()
  ...
  case class Score(ZJHM2:String,SQK2:Double,STOTAL2:Double)
  ...
  def getAgeScore(age:Int):Double ={
    if       ((age ==20) && (age==21)) 10.0
    else if ((age ==19) && (age==22) && (age==23)) 8.0
    else if ((age ==18) && (age==24) ) 6.0
    else if ((age ==17) && (age==25) && (age==26)) 5.0
    else if ((age >=27) && (age<=35)) 3.0
    else if ((age ==16)) 3.0
    else if ((age >=36) && (age<=200)) 1.0
    else if ((age ==15)) 1.0
    else 0.0
