...
object P_DCNW_5 {
  ...
  def compute(sqlContext:SQLContext,T_ZDRY_JBXX:DataFrame,HX_A_AJJBQK:DataFrame,VW_WSBA_HX_R_XYRC:DataFrame
    ...
    val pqlbQk = HX_A_AJJBQK.rdd.filter{
      r=>
        ...
        if ((zabmc.contains("盗车"))&&((ajztdm=="204")||(ajztdm=="299"))) true
        else false
    }.map{
      r=>
    ...
    val tJoinAreaJoinQkJoinWbJoinSbJoinJdcJoinJsz = tJoinAreaJoinQkJoinWbJoinSb.join(T_JDC,tJoinAreaJoinQkJoinWbJoinSb("ZJHM")===T_JDC("JDCZJHM"),"left").
      join(V_JSZ,tJoinAreaJoinQkJoinWbJoinSb("ZJHM")===V_JSZ("JSZZJHM"),"left")

    val res = tJoinAreaJoinQkJoinWbJoinSbJoinJdcJoinJsz.mapPartitions{
      Partition=>{
        Partition.map{
             p=>
               ...
               val scoreDynamc = getDynamcScore(Any2Int(p.getAs[Any]("NIGHTCNT")),Any2Int(p.getAs[Any]("CROSSAREACNT")),
                 p.getAs[Any]("JDCZJHM"),p.getAs[Any]("JSZZJHM"),p.getAs[Any]("SBZJHM"))
               val scoreTotal = scoreSex * (scoreAge + scoreArea + scoreQK ) * scoreDynamc;
               //Score4(zjhm,scoreAge,scoreSex,scoreArea,scoreQK,scoreDynamc,scoreTotal)
  ...
  case class Score4(Zjhm:String,S_age:Double,S_sex:Double,S_area:Double,S_qk:Double,S_dynamc:Double,S_total:Double)
  //fields:证件号码,前科分数,总积分
  case class Score(ZJHM5:String,SQK5:Double,STOTAL5:Double)
  def getDynamcScore(nightCnt:Int,crossAreaCnt:Int,jdc:Any,jsz:Any,sbzjhm:Any):Double = {
    var vScore_dynamc =0.0
    if ( nightCnt >= 3 ) vScore_dynamc += 3.0
    if ( crossAreaCnt >=3 ) vScore_dynamc += 2.0
    if (( nightCnt >=3 )&&( crossAreaCnt >=3 )) vScore_dynamc += 10.0
    if ( jdc != null )  vScore_dynamc += 3.0
    if ( jsz !=null ) vScore_dynamc += 4.0
    if ( sbzjhm != null ) vScore_dynamc += (-50.0)
    vScore_dynamc
  }
  ...
  def getAgeScore(age:Int):Double ={
    if ((age >=0) && (age<=20)) 8.0
    else if ((age >=21) && (age<=35)) 10.0
    else if ((age >=36) && (age<=44)) 8.0
    else if ((age >=45) && (age<=200)) 0.0
    else 0.0
  }
