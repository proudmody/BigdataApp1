create or replace procedure P_zdry_jmzp is  --重点人员街面诈骗
...
begin
     ...
     insert into T_ZDRY_LOG (ID,STARTTIME,  TABLENAME, DETAILACTION)
            values(vID,vStartTime,   'PRIAPWEB.t_zdry_jbxx_old'， '街面诈骗数据');
     commit;
     ...
     for user_info in cur_user_info  loop
          ...
          if vAge between 0 and 20 then
             vScore_age := 5.0;
          elsif vAge between 21 and 47 then
             vScore_age := 10.0;
          elsif vAge between 48 and 59 then
             vScore_age := 5;
          elsif vAge between 60 and 200 then
             vScore_age := 1.0;
          end if;
          ...
          vSql := 'select count(1) from d_zdry_gwdy_jmzp t where t.dm = '''||user_info.jg||'''' ;
          execute immediate vSql into isCountNum;
          if isCountNum >0 then
             vSql := 'select t.score from d_zdry_gwdy_jmzp t where t.dm = '''||user_info.jg||'''' ;
...
/*        --从业务汇集库中查询，正式抽取数据时用
          --计算街面诈骗前科 数据
          vSql := 'select count(*) from wsba.hx_a_ajjbqk@ywhj132 w where w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = '''||user_info.zjhm||''') and (zabmc like ''%诈骗%'' and afdcsmc in (select mc from d_zdry_jmzpmc) ) and AJZTDM in (''204'', ''299'')';
          ...
          if vNum_this > 0 then
            for v_criminal_record in (select guid from wsba.hx_a_ajjbqk@ywhj132 w where w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = user_info.zjhm) and (afdcsmc in (select mc from d_zdry_jmzpmc) and zabmc like '%诈骗%' ) and AJZTDM in ('204', '299')) loop
                begin
                    ...
                    values (user_info.rid,user_info.zjhm,user_info.xm, 5,vLastTime, '街面诈骗前科',
...
*/
          --临时
          vSql := 'select count(*) from t_ywhj_jmzp_template w where w.ajbh in (select distinct(ajbh) from vw_wsba_hx_r_xyrc@ywhj132 v where ajbh is not null and v.sfzhm = '''||user_info.zjhm||''')' ;
          execute immediate vSql into vNum_this;
          ...
          vLastTime := to_char(sysdate,'yyyymmddhh24miss');
          if vNum_this > 0 then
            for v_criminal_record in (select guid from t_ywhj_jmzp_template w where w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = user_info.zjhm) ) loop
                begin
                    insert into t_zdry_detail_result ( ID , IDENTITYCARD_NO,PERSON_NAME, FIELD_SCORE, INSERT_TIME, FIELD_TYPE, FIELD_DETAIL, FIELD_VALUE ,TABLENAME)
                    values (user_info.rid,user_info.zjhm,user_info.xm, 5,vLastTime, '街面诈骗前科',
                    'select * from wsba.hx_a_ajjbqk@ywhj132 w where w.guid = '''||v_criminal_record.guid||'''',
          ...
          insert into t_zdry_score_result (ZJHM,SCORE3,SCORE_OF_QK3,ZHGXSJ_JMZP)
