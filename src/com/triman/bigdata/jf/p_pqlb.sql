create or replace procedure P_zdry_pqlb is  --重点人员扒窃拎包   改进版本
...
begin
     ...
     for user_info in cur_user_info  loop
          ...
          --计算前科 数据
          vSql := 'select count(*) from wsba.hx_a_ajjbqk@ywhj132 w where (zabmc like ''%拎包%'' or zabmc like ''%扒窃%'') and AJZTDM in (''204'', ''299'') and w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = '''||user_info.zjhm||''')';
          ...
          if vNum_this > 0 then
            for v_criminal_record in (select guid from wsba.hx_a_ajjbqk@ywhj132 w where (zabmc like '%拎包%' or zabmc like '%扒窃%') and AJZTDM in ('204', '299') and w.ajbh in (select ajbh from vw_wsba_hx_r_xyrc@ywhj132 v where v.sfzhm = user_info.zjhm)) loop
                begin
                    ...
                    values (user_info.rid,user_info.zjhm,user_info.xm, 5,vLastTime, '扒窃拎包前科',
          ...
          --根据年龄积分
          if vAge between 0 and 15 then
             vScore_age := 1.0;
          elsif vAge between 16 and 17 then
             vScore_age := 2.0;
          elsif vAge between 18 and 25 then
             vScore_age := 10.0;
          elsif vAge between 26 and 35 then
             vScore_age := 8.0;
          elsif vAge between 36 and 44 then
             vScore_age := 5.0;
          elsif vAge between 45 and 79 then
             vScore_age := 3.0;
          elsif vAge between 80 and 200 then
             vScore_age := 0;
          end if;
          ...
          vSql := 'select count(1) from d_zdry_gwdy_pqlb t where t.dm = '''||user_info.jg||'''' ;
          execute immediate vSql into isCountNum;
          if isCountNum >0 then
             vSql := 'select t.score from d_zdry_gwdy_pqlb t where t.dm = '''||user_info.jg||'''' ;
             execute immediate vSql into vScore_area;
          else
             vScore_area := 0;
          end if;

          --根据拎包扒窃犯罪前科，计算处理分。 需要改进，细化到年份
          ...
          --update t_zdry_jbxx t set score1 = vTotal, SCORE_OF_QK1 = vNum_this ,t.zhgxsj_pqlb = vLastTime where t.id = user_info.id;
          insert into t_zdry_score_result (ZJHM,SCORE1,SCORE_OF_QK1,ZHGXSJ_PQLB)
     ...
     insert into T_ZDRY_LOG (STARTTIME, ENDTIME, TOTALTIME, TABLENAME,RECORDCOUNT, DETAILACTION)
            values(vStartTime, vEndTime, round(to_number(to_date(vEndTime,'yyyymmddhh24miss') - to_date(vStartTime,'yyyymmddhh24miss'))*24*60*60), 'PRIAPWEB.t_zdry_jbxx_old'，vCount, '扒窃拎包数据');
     commit;
